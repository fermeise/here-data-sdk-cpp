...
bool OlpClient::OlpClientImpl::ValidateBaseUrl() const {
  return base_url_.locked(
      [](const thread::Atomic<std::string, MutexType, ReadLock>::value_type&
             base_url) {
        return base_url == "" ||
               (base_url.find(kHttpPrefix) != std::string::npos ||
